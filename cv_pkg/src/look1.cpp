...
int main(int argc, char** argv)
{
    ...
    while (true) {
        ...
        for (auto& cnt : cnts_red) {
            ...
            minRect.points(box);
            for (int i = 0; i < 4; i++) {
                line(frame, box[i], box[(i + 1) % 4], Scalar(0, 0, 255), 3); // 红色线条
            }

            // 在中心绘制十字架
            line(frame, Point(minRect.center.x + 10, minRect.center.y), Point(minRect.center.x - 10, minRect.center.y), Scalar(0, 0, 255), 2);
            line(frame, Point(minRect.center.x, minRect.center.y + 10), Point(minRect.center.x, minRect.center.y - 10), Scalar(0, 0, 255), 2);

            // 显示“red”标签
            putText(frame, "red", Point(rect.x, rect.y - 5), FONT_HERSHEY_SIMPLEX, 0.7, Scalar(0, 0, 255), 2);

            // 显示中心坐标和角度
            stringstream ss;
            ss << "中心: (" << minRect.center.x << ", " << minRect.center.y << ")"; // 中心坐标
            putText(frame, ss.str(), Point(minRect.center.x + 10, minRect.center.y - 20), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(0, 0, 255), 2);
            
            stringstream ss_angle;
            ss_angle << "角度: " << minRect.angle << "°"; // 旋转角度
            putText(frame, ss_angle.str(), Point(minRect.center.x + 10, minRect.center.y + 20), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(0, 0, 255), 2);
        }

        // 为蓝色物体绘制轮廓和边界矩形
        for (auto& cnt : cnts_blue) {
            Rect rect = boundingRect(cnt); // 获取边界矩形
            RotatedRect minRect = minAreaRect(cnt); // 获取旋转矩形
            double area = contourArea(cnt); // 计算轮廓面积
            if (area < 2000) continue; // 忽略小区域

            rectangle(frame, rect, Scalar(255, 0, 0), 2); // 蓝色矩形
            putText(frame, "blue", Point(rect.x, rect.y - 5), FONT_HERSHEY_SIMPLEX, 0.7, Scalar(255, 0, 0), 2);

            // 显示中心坐标和角度
            stringstream ss;
            ss << "中心: (" << minRect.center.x << ", " << minRect.center.y << ")"; // 中心坐标
            putText(frame, ss.str(), Point(minRect.center.x + 10, minRect.center.y - 20), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(255, 0, 0), 2);

            stringstream ss_angle;
            ss_angle << "角度: " << minRect.angle << "°"; // 旋转角度
            putText(frame, ss_angle.str(), Point(minRect.center.x + 10, minRect.center.y + 20), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(255, 0, 0), 2);
        }

        // 你可以为黄色和绿色物体添加类似的中心坐标和角度显示逻辑
