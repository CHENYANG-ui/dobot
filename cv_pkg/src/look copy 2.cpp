...
int main(int argc, char **argv)
{
    ...
    // 创建发布者
    ros::Publisher color_pub = nh.advertise<std_msgs::String>("detected_color", 1000);
    ros::Publisher center_pub = nh.advertise<geometry_msgs::Point>("detected_center", 1000);
    ros::Publisher angle_pub = nh.advertise<std_msgs::Float32>("detected_angle", 1000);

    // 打开默认摄像头
    VideoCapture cap(0); //相机端口
    if (!cap.isOpened())
    {
    ...
    cout << "摄像头分辨率: " << frame_width << "x" << frame_height << endl;
    ...
    while (ros::ok())
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            for (auto &cnt : contours_list[i])
            {
                ...
                color_msg.data = color_labels[i];
                color_pub.publish(color_msg);

                // 发布中心坐标
                geometry_msgs::Point center_msg;
                center_msg.x = minRect.center.x;
                center_msg.y = minRect.center.y;
                center_msg.z = 0; // 二维坐标，z轴为0
                center_pub.publish(center_msg);

                // 发布角度
                std_msgs::Float32 angle_msg;
                angle_msg.data = minRect.angle;
                angle_pub.publish(angle_msg);

                // 输出到终端
                ROS_INFO("检测到颜色: %s", color_msg.data.c_str());
                ROS_INFO("中心坐标: [x: %f, y: %f]", center_msg.x, center_msg.y);
                ROS_INFO("旋转角度: %f", angle_msg.data);
            }
        }
